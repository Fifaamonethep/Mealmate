...
<template>
  <div class="max-w-3xl mx-auto space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
          <Bell class="w-6 h-6 text-brand-600 dark:text-brand-400" />
          <span>{{ t('notifications.title') }}</span>
        </h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ t('notifications.sub') }}</p>
      </div>

      <button
        v-if="myNotifications.length > 0"
        @click="handleMarkAllRead"
        class="bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs px-3.5 py-2 rounded-xl font-semibold flex items-center gap-1.5 transition-all shadow-sm"
      >
        <CheckCheck class="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
    ...
    <div v-if="myNotifications.length > 0" class="space-y-3">
      ...
      >
        <div :class="['w-2.5 h-2.5 rounded-full mt-1.5 shrink-0', n.isRead ? 'bg-slate-400 dark:bg-slate-600' : 'bg-brand-500 animate-ping']"></div>
    ...
    <div v-else class="glass-card p-12 text-center text-slate-500 dark:text-slate-400 space-y-2">
      <Bell class="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto" />
